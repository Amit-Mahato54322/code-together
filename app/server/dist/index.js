...
function isCodeWithinLimit(value) {
...
function broadcastToRoom(roomId, message, excludedSocket) {
        ...
        try {
            ...
            if (message.type === "room:join") {
                ...
                const participantAlreadyConnected = [...(existingConnections ?? [])]
                    .some((connection) => socketParticipants.get(connection)?.participantId === participantId);
            ...
            if (message.type === "language:update") {
                ...
                const room = roomService.updateLanguage(joinedRoomId, message.language);
                if (!room) {
                    return;
                }
